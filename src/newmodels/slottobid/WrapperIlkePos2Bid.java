...
import modelers.bidtoposition.ilke.BucketPositionToBidModel;
...
public class WrapperIlkePos2Bid extends AbstractSlotToBidModel{

	BucketPositionToBidModel ilke;
	
	public WrapperIlkePos2Bid(Query query, int numOfSlots) {
		super(query);
		Set<Query> queries = new HashSet<Query>();
		queries.add(query);
		ilke = new BucketPositionToBidModel(queries, numOfSlots);
	}

	@Override
	public double getPrediction(double slot) {
		return ilke.getBid(_query, slot);
	}
	...
	public static void main (String args[]) {
		...
		WrapperIlkePos2Bid w = new WrapperIlkePos2Bid(query, 5);
