...
public class RegressionPosToPrClick extends NewAbstractPosToPrClick {

	protected ArrayList<Double> _positions , _prclicks;
	...
	public RegressionPosToPrClick (Set<Query> queryspace) {
		try {
			c = new RConnection();
	...
	public boolean updateModel(QueryReport queryreport) {
		...
		double[] pos = new double[_positions.size()];
		double[] prclicks = new double[_positions.size()];

		for(int i = 0; i < _positions.size(); i++) {
			pos[i] = _positions.get(i);
			prclicks[i] = _prclicks.get(i);
		}

		int[] queryInd1 = new int[_positions.size()];
		int[] queryInd2 = new int[_positions.size()];
		int[] queryInd3 = new int[_positions.size()];
		int[] queryInd4 = new int[_positions.size()];
		int[] queryInd5 = new int[_positions.size()];
		int[] queryInd6 = new int[_positions.size()];
		...
		try {
			...
			String model = "model = glm(prclicks ~ queryInd1 + queryInd2 + queryInd3 + queryInd4 + queryInd5 + queryInd6 + pos , family = quasibinomial(link = \"logit\"))";
