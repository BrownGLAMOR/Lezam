...
public class WEKAEnsembleCPCToBid extends AbstractCPCToBid {
	 ...
	 * 
	 */
	protected HashMap<String,AbstractCPCToBid> _models;
	protected HashMap<String,AbstractCPCToBid> _usableModels;
	 ...
	 */
	protected HashMap<Query,LinkedList<AbstractCPCToBid>> _ensemble;
	...
	public WEKAEnsembleCPCToBid(Set<Query> querySpace, int numPastDays, int ensembleSize, boolean borda, boolean ignoreNaN) {
		 ...
		 * Initialize Models
		 */
		_models = new HashMap<String, AbstractCPCToBid>();
		_usableModels = new HashMap<String,AbstractCPCToBid>();
		 ...
		 * Initialize Ensemble
		 */
		_ensemble = new HashMap<Query, LinkedList<AbstractCPCToBid>>();
		_ensembleError = new HashMap<Query, LinkedList<Double>>();
		for(Query query : _querySpace) {
			LinkedList<AbstractCPCToBid> queryEnsemble = new LinkedList<AbstractCPCToBid>();
	...
	public void initializeEnsemble() {
		...
		for(int numDays = 20; numDays <= 60; numDays += 20) {
			for(double weight = 1.0; weight > .89; weight -= .1) {
				for(int i = 1; i < 8; i++) {
					addModel(new WEKACPCToBid(i,weight,numDays));
					addModel(new WEKACPCToBidPerQuery(_querySpace,i,weight,numDays));
				}
			}
		}
			
//				for(int i = 1; i < 20; i++) {
//					addModel(new WEKAClassifCPCToBid(i));
//				}
	}
	...
	public boolean updateModel(QueryReport queryReport,SalesReport salesReport, BidBundle bidBundle) {
		...
		for(String name : _models.keySet()) {
			AbstractCPCToBid model = _models.get(name);
	...
	public void updatePredictions(QueryReport queryReport) {
		...
		for(String name : _usableModels.keySet()) {
			AbstractCPCToBid model = _usableModels.get(name);
			HashMap<Query,Double> predictions = new HashMap<Query, Double>();
			for(Query query : _querySpace) {
				predictions.put(query, model.getPrediction(query, queryReport.getCPC(query)));
			}
		...
		for(Query query : _querySpace) {
			double prediction = getPrediction(query, queryReport.getCPC(query));
	...
	public void updateEnsemble(QueryReport queryReport, SalesReport salesReport, BidBundle bundle) {
		...
		if(ensembleReady) {
			...
			for(Query query : _querySpace) {
				...
				LinkedList<AbstractCPCToBid> queryEnsemble = _ensemble.get(query);
				for(AbstractCPCToBid model : queryEnsemble) {
			...
			for(String name : _models.keySet()) {
				...
				for(Query query : _querySpace) {
					...
					if(predictions != null) {
						...
						double ensembleerror = ensemblepred-bid;
						if(Double.isNaN(error)) {
							error = queryReport.getCPC(query);
						}
						double modelDiff = error-bid;
	...
	public String toString() {
		return "WEKAEnsembleCPCToBid(" + "numPastDays: " + NUMPASTDAYS + ", ensemble size: " + ENSEMBLESIZE + ", borda count: " + _borda + ", ignoreNaN: " + _ignoreNaN + ")";
	}
	...
	public void addModel(AbstractCPCToBid model) {
		addModel(model.toString(),model);
	}

	public void addModel(String name, AbstractCPCToBid model) {
	...
	public HashMap<Query, LinkedList<AbstractCPCToBid>> meanPredictionCombiner() {
		/*
		 ...
		 * Initialize Ensemble
		 */
		HashMap<Query, LinkedList<AbstractCPCToBid>> ensemble = new HashMap<Query, LinkedList<AbstractCPCToBid>>();
		_ensembleWeights = new HashMap<Query, HashMap<String,Double>>();
		for(Query query: _querySpace) {
			LinkedList<AbstractCPCToBid> queryEnsemble = new LinkedList<AbstractCPCToBid>();
			ArrayList<ModelErrorPair> modelErrorPairList = modelErrorMap.get(query);
			...
			for(ModelErrorPair modelErrorPair : modelErrorPairList) {
				...
				queryEnsemble.add((AbstractCPCToBid) modelErrorPair.getModel());
	...
	public HashMap<Query, LinkedList<AbstractCPCToBid>> bordaCountCombiner() {
		/*
		 * Initialize Error Mappings
		 */
		HashMap<Query,ArrayList<ModelErrorPair>> modelErrorMap = new HashMap<Query, ArrayList<ModelErrorPair>>();
		...
		for(Query query: _querySpace) {
			...
			for(ModelBordaPair modelBordaPair : modelBordaList) {
				if(i < ENSEMBLESIZE) {
					queryEnsemble.add((AbstractCPCToBid) modelBordaPair.getModel());
	...
	public AbstractModel getCopy() {
		return new WEKAEnsembleCPCToBid(_querySpace, NUMPASTDAYS, ENSEMBLESIZE, _borda, _ignoreNaN);
	}
