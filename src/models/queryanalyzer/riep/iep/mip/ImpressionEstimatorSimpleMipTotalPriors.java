 ...
 *
 */
public class ImpressionEstimatorSimpleMipTotalPriors extends
...
//	}
	...
	public ImpressionEstimatorSimpleMipTotalPriors(QAInstanceAll inst, double timeout, IloCplex cplex) {
		...
		this.totalImpsFirstSlotPrior = inst.getTotalImpsFirstSlot();
		//PRIORS PRINT HERE
...
// change objective
	...
	protected void postObjective(IloCplex cplex, CPlexVariables vars, int advertisers, int slots, int M) throws IloException {
		 ...
		 * 
		 */
		IloNumVar error = cplex.numVar(0, M);
		//System.out.println("Slots:"+slots);
		
		
		cplex.addLe(vars.S_a[0], cplex.sum(totalImpsFirstSlotPrior , error));
		cplex.addGe(vars.S_a[0], cplex.diff(totalImpsFirstSlotPrior , error));
		
		
	
		cplex.addMinimize(error);
...
//		cplex.addMaximize(allTs);	
	...
	public String getName() {
		return "SimpleMipExact-TotalPriors";
	}
