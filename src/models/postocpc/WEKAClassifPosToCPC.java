...
public class WEKAClassifPosToCPC extends AbstractPosToCPC {
   ...
   public WEKAClassifPosToCPC(int idx) {
      _idx = idx;
...
//			e1.printStackTrace();
   ...
   public double getPrediction(Query query, double pos) {
      ...
      Enumeration enumerCPC = _filteredData.attribute(1).enumerateValues();
      String[] enumCPCStrings = new String[_filteredData.attribute(1).numValues()];
      int countCPC = 0;
      while (enumerCPC.hasMoreElements()) {
         enumCPCStrings[countCPC] = (String) enumerCPC.nextElement();
         countCPC++;
      }
      ArrayList<Double> tokenVals = new ArrayList<Double>();
      for (int i = 0; i < countCPC; i++) {
         StringTokenizer tokenizer = new StringTokenizer(enumCPCStrings[i], "'()-[]");
         String token1 = tokenizer.nextToken();
         String token2;
         if (tokenizer.hasMoreTokens()) {
            token2 = tokenizer.nextToken();
      ...
      try {
         ...
         prediction = 0;
         double[] CPCArr = _data.attributeToDoubleArray(1);
         for (int i = 0; i < CPCArr.length; i++) {
            int predIdx = 0;
            double CPC = CPCArr[i];
            if (CPC == 0) {
               continue;
            ...
            for (int j = 0; j < tokenVals.size() - 1; j++) {
               if (CPC >= tokenVals.get(j) &&
                       CPC <= tokenVals.get(j + 1)) {
                  predIdx = j;
                  break;
               }
            }
            if (nominalCounts[predIdx] != 0) {
               prediction += (predArr[predIdx] / ((double) nominalCounts[predIdx])) * CPC;
            }
         }
//			System.out.println("bid: " + bid + ", prediction: " +prediction + ", mean or mode: ");
   ...
   public String toString() {
      switch (_idx) {
         case 1:
            return "WEKAClassifPosToCPC(BayesNet)";
         case 2:
            return "WEKAClassifPosToCPC(NaiveBayes)";
         case 3:
            return "WEKAClassifPosToCPC(IBk)";
         case 4:
            return "WEKAClassifPosToCPC(KStar)";
         case 5:
            return "WEKAClassifPosToCPC(LWL)";
         case 6:
            return "WEKAClassifPosToCPC(AttributeSelectedClassifier)";
         case 7:
            return "WEKAClassifPosToCPC(Bagging)";
         case 8:
            return "WEKAClassifPosToCPC(LogitBoost)";
         case 9:
            return "WEKAClassifPosToCPC(RandomCommittee)";
         case 10:
            return "WEKAClassifPosToCPC(DecisionTable)";
         case 11:
            return "WEKAClassifPosToCPC(DTNB)";
         case 12:
            return "WEKAClassifPosToCPC(NNge)";
         case 13:
            return "WEKAClassifPosToCPC(OneR)";
         case 14:
            return "WEKAClassifPosToCPC(PART)";
         case 15:
            return "WEKAClassifPosToCPC(J48)";
         case 16:
            return "WEKAClassifPosToCPC(J48graft)";
         case 17:
            return "WEKAClassifPosToCPC(NBTree)";
         case 18:
            return "WEKAClassifPosToCPC(RandomForest)";
         case 19:
            return "WEKAClassifPosToCPC(REPTree)";
         default:
            return "WEKAClassifPosToCPC(NaiveBayes)";
      }
   }
...
//			e1.printStackTrace();
   ...
   public Classifier getClassifier(int idx) {
      ...
      switch (idx) {
         ...
         case 5:
            ...
            break;
         case 6:
            classifier = (Classifier) new AttributeSelectedClassifier();
            break;
         case 7:
         ...
         case 8:
            ...
            break;
         case 9:
            classifier = (Classifier) new RandomCommittee();
            break;
         case 10:
            classifier = (Classifier) new DecisionTable();
            break;
         case 11:
            classifier = (Classifier) new DTNB();
            break;
         case 12:
            classifier = (Classifier) new NNge();
            break;
         case 13:
            classifier = (Classifier) new OneR();
            break;
         case 14:
            classifier = (Classifier) new PART();
            break;
         case 15:
   ...
   public AbstractModel getCopy() {
      return new WEKAClassifPosToCPC(_idx);
   }
}
