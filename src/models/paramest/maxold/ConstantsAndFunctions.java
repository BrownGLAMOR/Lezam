...
public abstract class ConstantsAndFunctions {
	...
	public static final double[][] getCubicRoots(double a, double b, double c, double d) {
		...
		if (D >= 0) {
			...
			roots[1][0] = 2 * Math.sqrt(-Q)
					* Math.cos((theta + 2 * Math.PI) / 3.0) - A / 3.0;
			roots[2][0] = 2 * Math.sqrt(-Q)
					* Math.cos((theta + 4 * Math.PI) / 3.0) - A / 3.0;
		}
		return roots;
	}
	...
	public static double solve(double[] coefficientsArray) {
		...
		/*
		 * for (int i = 0; i < 5; i++) {
		 * System.out.println("The coefficients array is: "
		 * +coefficientsArray[i]); }
		 */
...
//	}
	...
	//
	// /*
	// * QueryReport/SalesReport report contain information about the overall
	// number of
	// * impressions and clicks that we saw
	// *
	// * impressionsPerSlot contains the number of impressions we saw in each
	// slot with
	// * index 0 being the highest slot and the size being the last slot
	// *
	// * advertisersAbovePerSlot is a list that containts the advertisers that
	// were above
	// * us when we were in any given slot, the first index corresponds to the
	// same slot as
	// * the impressionsPerSlot varaible
	// *
	// * UserStates contains the actual number of users in ever product in every
	// state
	// *
	// * ads containts the ad that each advertiser placed. The strings in this
	// hashmap are
