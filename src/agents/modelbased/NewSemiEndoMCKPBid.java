 ...
 */
public class NewSemiEndoMCKPBid extends AbstractAgent {
   ...
   private HashMap<Query, Integer> _totalImpressions;

   public NewSemiEndoMCKPBid() {
      this(false, false, false);
   }


   public NewSemiEndoMCKPBid(boolean forward, boolean pricelines, boolean budget) {
      BUDGET = budget;
...
//		_R.setSeed(616866);
   ...
   public Set<AbstractModel> initModels() {
      ...
      ParticleFilterAbstractUserModel userModel = new jbergParticleFilter(0.004932699,
                                                                          ...
                                                                          0.188113883,
                                                                          0.220140091);
      AbstractQueryToNumImp queryToNumImp = new NewBasicQueryToNumImp(userModel);
      ...
      NewBasicConvPrModel convPrModel = new NewBasicConvPrModel(userModel, _querySpace, _baseConvProbs);
      models.add(queryAnalyzer);
   ...
   protected void buildMaps(Set<AbstractModel> models) {
      for (AbstractModel model : models) {
         if (model instanceof AbstractQueryAnalyzer) {
            ...
            ParticleFilterAbstractUserModel userModel = (ParticleFilterAbstractUserModel) model;
      ...
      for (AbstractModel model : _models) {
         if (model instanceof AbstractQueryAnalyzer) {
            ...
            for (Query q : _querySpace) {
               ...
               _totalImpressions.put(q, totalImps);
            }
         } else if (model instanceof ParticleFilterAbstractUserModel) {
            ParticleFilterAbstractUserModel userModel = (ParticleFilterAbstractUserModel) model;
            userModel.updateModel(_totalImpressions);
         } else if (model instanceof AbstractQueryToNumImp) {
            AbstractQueryToNumImp queryToNumImp = (AbstractQueryToNumImp) model;
   ...
   public String toString() {
      return "NewSemiEndoMCKPBid(Budget: " + BUDGET + ", Forward Update: " + FORWARDUPDATING + ", Pricelines: " + PRICELINES + ")";
   }

   @Override
   public AbstractAgent getCopy() {
      return new NewSemiEndoMCKPBid(FORWARDUPDATING, PRICELINES, BUDGET);
   }
}
