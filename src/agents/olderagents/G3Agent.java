...
public class G3Agent extends AbstractAgent {
   ...
   public BidBundle getBidBundle(Set<AbstractModel> models) {
      ...
      if (BUDGET) {
         _bidBundle.setCampaignDailySpendLimit(1000);
      }
      ...
      for (Query query : _querySpace) {
         ...
         if (query.getType() == QueryType.FOCUS_LEVEL_ONE) {
            if (_manSpecialty.equals(query.getManufacturer())) {
               _estimatedPrice.put(query, 15.0);
            } else {
               if (query.getManufacturer() != null) {
                  _estimatedPrice.put(query, 10.0);
               } else {
                  _estimatedPrice.put(query, 10.0 + 5 / 3);
               }
            }
      ...
      if (_day > 1 && salesReport != null && queryReport != null) {
         ...
         for (Query query : _querySpace) {
            if (queryReport.getImpressions(query) > 0) {
               _prClick.put(query, queryReport.getClicks(query) * 1.0 / queryReport.getImpressions(query));
            }
         }
   ...
   protected void printInfo() {
      ...
      for (Query q : _querySpace) {
         ...
         if (_salesReport.getConversions(q) > 0) {
            ...
            buff.append("\t").append("Revenue: ").append("0.0").append("\n");
         }
         if (_queryReport.getClicks(q) > 0) {
            ...
            buff.append("\t").append("Conversion Pr: ").append("No Clicks").append("\n");
         }
         ...
         if (_salesReport.getConversions(q) > 0) {
            ...
            buff.append("\t").append("Profit: ").append("0").append("\n");
         }
