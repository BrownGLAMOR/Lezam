   ...
   public BidBundle getBidBundle(Set<AbstractModel> models) {
      ...
      for (Query query : _querySpace) {
         ...
         while (bid <= maxBid) {
            double prClicks = bidToPrClickModel.getPrediction(query, bid,
                                                              null);
            if (TARGET) {
               prClicks = targetModel.getClickPrPrediction(query, prClicks, false);
            }
            ...
            if (ADJUSTMENT) {
               maxClicks = Math.min((_queryReport.getClicks(query) + 1) * 5, prClicks * imp);
            }
      ...
      while (i > 0) {
         ...
         if (sales[i][capacity] > 0) {
            ...
            if (Double.isNaN(prConv) || prConv == 0) {
               prConv = _baselineConv.get(queries[i]);
            }
   ...
   public void initBidder() {
      ...
      for (Query q : _querySpace) {
         if (q.getType() == QueryType.FOCUS_LEVEL_ZERO) {
            _baselineConv.put(q, 0.1);
         }
         if (q.getType() == QueryType.FOCUS_LEVEL_ONE) {
            if (q.getComponent() == _compSpecialty) {
               _baselineConv.put(q, 0.27);
            } else {
               _baselineConv.put(q, 0.2);
            }
         }
         if (q.getType() == QueryType.FOCUS_LEVEL_TWO) {
            if (q.getComponent() == _compSpecialty) {
               _baselineConv.put(q, 0.39);
            } else {
               _baselineConv.put(q, 0.3);
            }
         }
      }
      ...
      for (Query query : _querySpace) {
         ...
         if (query.getType() == QueryType.FOCUS_LEVEL_ONE) {
            if (_manSpecialty.equals(query.getManufacturer())) {
               revenues.put(query, 15.0);
            } else {
               if (query.getManufacturer() != null) {
                  revenues.put(query, 10.0);
               } else {
                  revenues.put(query, 10.0 + 5 / 3);
               }
            }
   ...
   protected void printInfo() {
      ...
      for (Query q : _querySpace) {
         ...
         if (_salesReport.getConversions(q) > 0) {
            ...
            buff.append("\t").append("Conversions Pr: ").append("No click").append("\n");
         }
         ...
         if (_queryReport.getClicks(q) > 0) {
            ...
            buff.append("\t").append("PPC: ").append("No click").append("\n");
         }
         buff.append("****************\n");
      }
