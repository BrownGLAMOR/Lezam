...
//				for (int queryIdx=11; queryIdx<=11; queryIdx++) {
				for (int queryIdx=0; queryIdx<numQueries; queryIdx++) {
					...
					for (int i=0; i<reducedSampledAvgPos.length; i++) if(Double.isNaN( reducedSampledAvgPos[i] )) hasNaN = true;
					if (hasNaN) continue;
					//FIXME END DEBUG
...
//					for (int ourAgentIdx=0; ourAgentIdx<=0; ourAgentIdx++) {
						...
						if(impressionEstimatorIdx == 1) {
							...
							if (SAMPLED_AVERAGE_POSITIONS) {
								for (int i=0; i<reducedAvgPos.length; i++) {
									if (i == ourAgentIdx) avgPos[i] = reducedAvgPos[i];
									else avgPos[i] = reducedSampledAvgPos[i];
								}
	...
	private double[] getAgentImpressionsDistributionMeanOrStdev(GameStatus status, Query query, boolean getMean) {
		...
		for (int a=0; a<agents.length; a++) {
			...
			if (getMean) meanOrStdevImps[a] = meanAndStdev[0];
			else meanOrStdevImps[a] = meanAndStdev[1];
		}
	...
	private double[] getAgentImpressionsDistributionMean2(GameStatus status, Query query) {
		...
		for (int a=0; a<agents.length; a++) {
			...
			if (daysWithImps > 0) meanImps[a] = aggregateImps / (double) daysWithImps;
			else meanImps[a] = Double.NaN;
		}
		return meanImps;
	}
